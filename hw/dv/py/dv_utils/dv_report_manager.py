...
"""Centralized SV-UVM style reporting manager for Python DV."""
...
from dv_utils.dv_report_catcher import dv_report_catcher, dv_report_message
...
class DvReportStats:
    ...
    def total_fails(self, policy: DvReportPolicy) -> int:
        total = 0
    ...
    def error_quit_reached(self, policy: DvReportPolicy) -> bool:
        return int(policy.max_quit_count) > 0 and self.error_quit_count >= int(policy.max_quit_count)


class DvReportFilter(logging.Filter):
    """Filter that normalizes severities and updates global counters."""

    def __init__(self, manager: "DvReportManager") -> None:
        super().__init__("dv_report_filter")
...
class DvWrappedFormatter(logging.Formatter):
...
class DvReportManager:
    """Singleton manager that emulates SV report_server/report_catcher behavior."""

    _instance: Optional["DvReportManager"] = None
    ...
    def __init__(self) -> None:
        ...
        self._policy = DvReportPolicy()
        self._stats = DvReportStats()
        self._catcher = dv_report_catcher("uvm_report_catcher")
        self._filter = DvReportFilter(self)
        self._attached: list[tuple[Any, bool, Optional[logging.Formatter]]] = []
        self._attached_keys: set[tuple[str, int]] = set()
        self._initialized = False

    @classmethod
    def get(cls) -> "DvReportManager":
        if cls._instance is None:
            cls._instance = DvReportManager()
        return cls._instance

    @classmethod
    def get_or_none(cls) -> Optional["DvReportManager"]:
        return cls._instance
    ...
    def create(
        ...
        verbosity: int = 100,
        policy: Optional[DvReportPolicy] = None,
    ...
    @property
    def policy(self) -> DvReportPolicy:
        return self._policy

    def get_stats(self) -> DvReportStats:
        return self._stats
    ...
    def initialize(
        ...
        self.shutdown()
        self._root_logger = root_logger if root_logger is not None else logging.getLogger()
        self._verbosity = int(verbosity)
        self._print_char_len = int(print_char_len)
        self._policy = policy if policy is not None else DvReportPolicy()
        self._stats.clear()
    ...
    def shutdown(self) -> None:
        for target, is_handler, formatter in self._attached:
            try:
                ...
                pass
            if is_handler and formatter is not None:
                try:
    ...
    def emit_uvm(
        ...
        extra = {"_dv_counted_by_emit": True}
        if report_id:
    ...
    def process_record(self, record: logging.LogRecord) -> None:
        if getattr(record, "_dv_report_processed", False):
            return
        setattr(record, "_dv_report_processed", True)

        # Records emitted through emit_uvm() are already catch-processed and counted.
        if getattr(record, "_dv_counted_by_emit", False):
            return
    ...
    def failure_message(self, context: str = "") -> Optional[str]:
    ...
    def log_final_status(
        ...
        context: str = "",
        prior_failure_msg: Optional[str] = None,
    ) -> Optional[str]:
    ...
    def register_logger(self, log: Optional[logging.Logger]) -> None:
        if log is None:
            return
        ...
        for handler in log.handlers:
            ...
            if original_formatter is not None and self._print_char_len > 0:
                if isinstance(original_formatter, DvWrappedFormatter):
                    wrapped = DvWrappedFormatter(
                        original_formatter.base_formatter,
                        self._print_char_len,
                    )
                    original_formatter = original_formatter.base_formatter
                else:
                    wrapped = DvWrappedFormatter(original_formatter, self._print_char_len)
    ...
    def _apply_catcher(self, msg: str, severity: str, report_id: str = "") -> str:
        report = dv_report_message(report_id=report_id, message=msg, severity=severity)
